...
filedir = r''  # 数据存储路径
filename = os.path.join(filedir, 'Data_10sec_For_Instance.pickle')  # 输出数据文件名
...
# 识别SINGLE_GLOBAL_SPEED和加速限制
SpeedLimit = {0: 10.4, 1: 12, 2: 120./3.6, 3: 180./3.6, 4: 120/3.6}
# ACC的在线来源：步行：1.5火车1.15，公交车。1.25(2)，自行车：2.6，火车：1.5
...
# Total_Instance_InSequence检查所有用户中每个实例的GPS点数
Total_Instance_InSequence = []
# Total_Motion_Instance：每个元素是一个数组，包含每个实例的四个通道
...
for z in tqdm(range(len(Trajectory_Label_Array))):
    ...
    # InstanceNumber：将用户轨迹分解为实例。统计每个实例的GPS点数
    delta_time = []
    tempSpeed = []
    ...
    # 由于没有最后一个点的数据，我们假设Delta_Time为user guide中的平均时间
    # (即，3秒)，速度为tempSpeed等于到目前为止的最后时间。
    delta_time.append(3)
    tempSpeed.append(tempSpeed[len(tempSpeed) - 1])

    # InstanceNumber：标明每个实例的长度
    InstanceNumber = []
    ...
    for k in range(len(InstanceNumber)):  # InstanceNumber是一个轨迹中每个轨迹的长度，len(InstanceNumber)是轨迹的数量
        Data = Data_All_Instance[k]
        day_of_week = math.floor(Data[0, 2]) % 7  # 计算星期几
        start_time = math.floor((Data[0, 2] - math.floor(Data[0, 2])) * 24.0)  # 计算开始的时间点
        if InstanceNumber[k] != 0:
            Day_of_Week.append(day_of_week)
        ...
        Temp_RD = []
        outlier = []
        for i in range(len(Data) - 1):  # 求Bearing和Speed
            A = (Data[i, 0], Data[i, 1])
            B = (Data[i+1, 0], Data[i+1, 1])
            # Temp_RD.append(vincenty(xi, xj).meters)
            Temp_RD.append(distance.distance(A, B).meters)
            Delta_Time[k].append((Data[i + 1, 2] - Data[i, 2]) * 24. * 3600 + 1)  # Add one second to prevent zero time，加一秒防止0速度
        ...
        Temp_Speed = [i for j, i in enumerate(Temp_Speed) if j not in outlier]  # 添加Speed
        ...
        Delta_Time[k] = [i for j, i in enumerate(Delta_Time[k]) if j not in outlier]  # 问题这边的多次调用非空的outlier的话，会导致变短
        ...
        Temp_ACC = []
        outlier = []
        for i in range(len(Speed[k]) - 1):  # 求当前文件的当前轨迹的加速度
        ...
        Temp_ACC = [i for j, i in enumerate(Temp_ACC) if j not in outlier]  # 排除有问题的加速度
        if len(Temp_ACC) < 10:
            InstanceNumber[k] = 0
            NoOfOutlier += 1
            continue
        ...
        # Compute Breating Rate from Bearing, and Velocity change from single_global_Speed
        for i in range(len(Bearing[k]) - 1):
            Diff = abs(Bearing[k][i+1] - Bearing[k][i])  # Bearing的差值的绝对值
            BearingRate[k].append(Diff)
        BearingRate[k].append(BearingRate[k][-1])

        for i in range(len(Speed[k]) - 1):  # 速度的差值的绝对值
            Diff = abs(Speed[k][i+1] - Speed[k][i])
            if Speed[k][i] != 0:
                Velocity_Change[k].append(Diff/Speed[k][i])
            else:
                Velocity_Change[k].append(1)
        Velocity_Change[k].append(Velocity_Change[k][-1])  # 都是结尾重复一下

        # Now we apply the smoothing filter on each instance:
        def savitzky_golay(y, window_size, order, deriv=0, rate=1):
            r"""Smooth (and optionally differentiate) x with a Savitzky-Golay filter.
            The Savitzky-Golay filter removes high frequency noise from x.
            It has the advantage of preserving the original shape and
            features of the signal better than other types of filtering
            approaches, such as moving averages techniques.
            Parameters
            ----------
            y : array_like, shape (num,)
            ...
            Returns
            -------
            ys : ndarray, shape (num)
                the smoothed signal (or it's n-th derivative).
            Notes
            -----
            The Savitzky-Golay is a type of low-pass filter, particularly
            suited for smoothing noisy x. The main idea behind this
            ...
            References
            ----------
            .. [1] xi. Savitzky, M. J. E. Golay, Smoothing and Differentiation of
               Data by Simplified Least Squares Procedures. Analytical
               Chemistry, 1964, 36 (8), pp 1627-1639.
            .. [2] Numerical Recipes 3rd Edition: The Art of Scientific Computing
               W.H. Press, S.xi. Teukolsky, W.T. Vetterling, xj.P. Flannery
            ...
            try:
                ...
                window_size = np.abs(np.array(window_size).astype(np.int8))
                # order = np.abs(np.int(order))
                order = np.abs(np.array(order).astype(np.int8))
            except ValueError:
            ...
            firstvals = y[0] - np.abs(y[1:half_window + 1][::-1] - y[0])  # -1是反向取值
    ...
    Total_Start_Time.append(Start_Time)
...
with open(os.path.join(filedir, '10s_Revised_InstanceCreation+NoJerkOutlier+Smoothing.pickle'), 'wb') as f:
    pickle.dump([Total_RelativeDistance, Total_Speed, Total_Acceleration, Total_Jerk, Total_BearingRate, Total_Label,
                 Total_InstanceNumber, Total_Instance_InSequence, Total_Delta_Time, Total_Velocity_Change, Total_Day_of_Week, Total_Start_Time], f)
