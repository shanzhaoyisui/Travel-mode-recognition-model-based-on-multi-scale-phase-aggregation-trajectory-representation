...
# os.chdir(r'E:\1 machine learning program\My TMI Project\operating console\single sample')
os.chdir(r'\Combined Trajectory_Label_Geolife')  # 原始数据存储路径
target_path = r''  # 数据存储路径
...
Mode_Index = {"walk": 0, "run": 0, "bike": 1, "bus": 2, "car": 3, "taxi": 3, "subway": 4, "railway": 4,
              "train": 4, "motocycle": 8, "boat": 9, "airplane": 9, "other": 9}
# Mode_Index = {"walk": 0, "run": 0, "bike": 1, "bus": 2, "car": 3, "taxi": 3, "subway": 4, "railway": 4,
#               "train": 4, "motocycle": 5, "boat": 6, "airplane": 6, "other": 6}
...
# 1
for k in tqdm(range(len(UserNon))):
...
# StartTime and EndTime in days after 1899/12/30 for each x point in labels.cv
...
# LabelMatrix = the array that has Start time(days), End time(days), and labels
    ...
    for index, row in enumerate(LabelMatrix):
        ...
        # 这里的Dates是个二维的数列，所以输出结果是个二维的，所以下边是A[0]
        B = np.where(Dates <= (float(row[1]) + Sec))
        C = list(set(A[0]).intersection(B[0]))
        if len(C) == 0:
            print("error")
        [Mode_Trajectory.append(row[2]) for i in C]  # row[2]是标签，C是空的就不加，C是几个轨迹段就加几个
        [C_list.append(i) for i in C]  # 加进去的是索引值
...
with open(os.path.join(target_path, r"Revised_Trajectory_Label_Array.pickle"), 'wb') as f:  # Python 3: open(..., 'wb')
