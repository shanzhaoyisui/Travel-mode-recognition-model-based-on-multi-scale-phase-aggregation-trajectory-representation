...
class MNISTFeatureLayer(nn.Sequential):# MNIST特征层
...
class UCILetterFeatureLayer(nn.Sequential):# UCILetter特征层
    def __init__(self,dropout_rate=0.,shallow=True):
...
class UCIYeastFeatureLayer(nn.Sequential):# UCIYeast特征层
    def __init__(self,dropout_rate=0.,shallow=True):
...
class Tree(nn.Module):
    def __init__(self,depth,n_in_feature,used_feature_rate,n_class, jointly_training=True):
        ...
        self.feature_mask = Parameter(torch.from_numpy(self.feature_mask).type(torch.float64), requires_grad=False)
        # leaf label distribution
        if jointly_training:
            self.pi = np.random.rand(self.n_leaf,n_class)
            # self.pi = Parameter(torch.from_numpy(self.pi).type(torch.FloatTensor),requires_grad=True)# 转换成可学习参数
            self.pi = Parameter(torch.from_numpy(self.pi).type(torch.float64), requires_grad=True)
        else:
            ...
            self.pi = Parameter(torch.from_numpy(self.pi).type(torch.float64), requires_grad=False)
        # decision
        self.decision = nn.Sequential(OrderedDict([
                        ('linear1',nn.Linear(n_used_feature,self.n_leaf, dtype=torch.float64)),
                        ('sigmoid', nn.Sigmoid()),
                        ]))
